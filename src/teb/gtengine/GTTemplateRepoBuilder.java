...
import teb.gtengine.GTJavaBaseTesterImpl;
...
public class GTTemplateRepoBuilder {
	...
	public GTTemplateRepoBuilder withPreCompilerFactory(
	...
	public static class GTPreCompilerFactoryImpl implements
			GTPreCompilerFactory {
	...
	private GTTemplateRepo createTemplateRepo(
			final GTPreCompilerFactory preCompilerFactory) {
		...
		final GTTemplateRepo templateRepo = new GTTemplateRepo(getClass()
				.getClassLoader(), false, preCompilerFactory, false, null);
		return templateRepo;
	}
	 ...
	 */
	private static class SpecialGTTemplateLocationReal extends
			GTTemplateLocationReal {

		private final boolean fakeWindowsNewLines;

		private SpecialGTTemplateLocationReal(String relativePath,
				URL realFileURL, boolean fakeWindowsNewLines) {
	...
	private static class FolderResolver implements GTFileResolver.Resolver {
		...
		public GTTemplateLocationReal getTemplateLocationReal(String queryPath) {
			...
			try {
				return new SpecialGTTemplateLocationReal(queryPath, f.toURI()
						.toURL(), fakeWindowsNewLines);
		...
		@Override
		public GTTemplateLocationReal getTemplateLocationFromRelativePath(
				String relativePath) {
			// in our case, since we don't have stuff like '{modules: blah}/' in
			// our path, this is the same as queryPayj
	...
	public GTTemplateRepo build() {
		if (this.templateRootFolder == null) {
			...
			GTFileResolver.impl = new FolderResolver(templateRootFolder,
					fakeWindowsNewLines);
		}
